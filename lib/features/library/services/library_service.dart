...
class LibraryService {
  ...
  static Future<List<HadithModel>> fetchHadiths({
    ...
    if (cached != null) {
      try {
        ...
        // بيانات تالفة — نتجاهل ونعيد التحميل
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // التعامل مع بنيتين محتملتين للـ API
        if (data is List) {
          hadithsData = data;
        } else if (data is Map && data['hadiths'] is List) {
          hadithsData = data['hadiths'] as List;
        }

        // تطبيق النطاق (start, limit)
        final startIdx = (start - 1).clamp(0, hadithsData.length);
        final endIdx = (startIdx + limit).clamp(0, hadithsData.length);
        final slice = hadithsData.sublist(startIdx, endIdx);

        // تخزين مؤقت
        await prefs.setString(cacheKey, jsonEncode(slice));

        return slice.map((h) => HadithModel.fromJson(h)).toList();
      }
      return [];
    } catch (e) {
  ...
  static Future<HadithModel?> fetchHadith({
    ...
    try {
      ...
      debugPrint('fetchHadith error: $e');
      return null;
    }
  }
  ...
  static Future<List<HadithModel>> searchHadiths(String query) async {
    try {
      ...
      debugPrint('searchHadiths error: $e');
      return [];
    }
  }
}
